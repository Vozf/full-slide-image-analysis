...
# Support class for image showing
# TODO Refactor code with the same code convension
class ImageHelper:
    def __init__(self, fileName):
        self.openslideImage = openslide.OpenSlide(fileName)
        self.current_level = self.openslideImage.level_count - 1
        self.level_dimensions = self.openslideImage.level_dimensions
        ...
        self.imageSlide = openslide.ImageSlide(self.image)
        print self.imageSlide.level_downsamples
    ...
    def change_image_properties(self, coordinates, level, size):
        ...
        self.image = self.openslideImage.read_region(coordinates, self.current_level, size)
        return ImageQt.ImageQt(self.image)
    ...
    def zoom_out(self):
        if self.current_level < self.openslideImage.level_count - 1:
            self.current_level = self.current_level + 1
            self.scale_factor /= 2
            self.__calculate_movement_step_coordinates()
        return self.change_image_properties(self.current_coordinates, self.current_level, self.current_window_size)
    ...
    def move_down(self):
        if (self.current_coordinates[1] + 2 * self.current_movement_step[1]) <= self.image_dimensions[1]:
            self.current_coordinates = (
            self.current_coordinates[0], self.current_coordinates[1] + self.current_movement_step[1])
            print self.level_dimensions[self.current_level]
            print self.current_coordinates
